...
const CatalogPage = () => {
  ...
  const { filter } = useParams(); // Получаем параметр filter из URL
  ...
  useEffect(() => {
    const activeTab = CatalogFilterTabs.findIndex(
      (tab) => tab.title.toLowerCase() === filter
    );
    setActiveFilterTab(activeTab === -1 ? 0 : activeTab);
  }, [filter]); // Когда изменится параметр filter, обновим активный таб

  const handleTabClick = (index) => {
    setActiveFilterTab(index);
    navigate(`/catalog/${CatalogFilterTabs[index].title.toLowerCase()}`); // Меняем URL без перезагрузки страницы
  };
  ...
  const filteredData = data.filter(
    (item) =>
      CatalogFilterTabs[activeFilterTab].title === item.type ||
      activeFilterTab === 0
  );
          ...
          <div className={styles.catalogPage_inner}>
            ...
            <div className={styles.catalog_filter_tabs}>
              {CatalogFilterTabs.map(({ title, icon }, index) => {
                return (
                  <CatalogFilterTab
                    className={
                      activeFilterTab === index ? styles.activeTab : ""
                    }
                    key={index}
            ...
            <div className={styles.catalog}>
              <aside className={styles.catalog_filter}>
                <div className={styles.catalog_filter_title}>Фильтры</div>
                <div className={styles.filter_item_wrapper}>
                  <span className={styles.filter_item_title}>Тип</span>
                  <select
                ...
                <div className={styles.filter_item_wrapper}>
                  ...
                  <select className={styles.filter_item}>
                    <option>Свайный</option>
                  </select>
                </div>
                <div className={styles.filter_item_wrapper}>
                  <span className={styles.filter_item_title}>Стиль дома</span>
                  <select className={styles.filter_item}>
                    <option>Красивый</option>
                  </select>
                </div>
                <div className={styles.filter_item_wrapper}>
                  <span className={styles.filter_item_title}>Этажи</span>
                  <select className={styles.filter_item}>
                    <option>1</option>
                  </select>
                </div>
              ...
              <div className={styles.catalog_items}>
                {filteredData.length === 0 ? (
                  <div>Ошибка: Ничего не найдено!</div>
                ) : null}
